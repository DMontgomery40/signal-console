...
The "Sensitivity adapter" is not a new algorithm. It is the existing `kMad`-named control surface for the board model's innovation trigger. In the live runtime that control now feeds the Python state-space detector's enter threshold, not a literal rolling `median + K·MAD` rule. The live runtime default is `3.0`; `6.0` remains the calmer Backtest comparison preset. Current fires/game, known-case recall, and market-native outlier-alignment numbers come from the regenerated bakeoff artifacts and Known Cases page, not from this PRD prose, because the official incident corpus and tape-derived denominator change as the desk adds cases and the local coverage window moves.
...
**What it is.** A slider on the Backtest tab that adjusts the `kMad` trigger parameter for the board state-space model. In the current runtime it scales the positive standardized-innovation gate rather than a literal `median + K · MAD` threshold, but the familiar K-shaped control remains the trader-facing way to explore sensitivity.
...
1. **Canonical live default:** `K_MAD_LIVE = 3.0`, weighting `"volume"` — the sensitive board-lane trigger setting. This is what Recent, Live, and any non-Backtest board lane uses when it calls the Python state-space runtime. Justification: for a suspend-signal whose miss cost (an exposed bad market) exceeds the per-fire review cost, the higher-recall posture wins. Current empirical counts live in the generated bakeoff artifact.
...
4. **K is a compute parameter, not a persisted dimension.** The `board-mad` detector still iterates sanitized `quote_ticks` in time order, buckets by the configured `bucketSeconds`, and builds whole-board intensity from weighted implied-probability deltas. The live fire decision is now the Python state-space model: it predicts a latent baseline, scales the innovation by uncertainty, and fires when positive standardized innovation clears the K-shaped trigger after elapsed holdoff `warmupBuckets × bucketSeconds`. The live default anchor profile is `baselineMode="opening-ramp"`: first active alert checks compare against the opening elapsed duration `openingBaselineBuckets × bucketSeconds` and graduate to rolling memory at `openingRampCompleteBuckets × bucketSeconds`. Backtest and Settings also expose `baselineMode="historical-blend"`, which starts from last-five same-side NBA priors, fades them out by game-clock elapsed time, uses game-clock memory for the current-game baseline, and can add a short wall-clock recent tack-on for timeout/dead-ball betting bursts. **Nothing about K lives in the gold DB**; it is purely a knob inside the detector's compute loop.
...
- FR-8: The `board-mad` detector builds whole-board intensity from sanitized per-market implied-probability deltas, then feeds those bucket observations into the Python state-space runtime. The observation embedding can also add a tunable cross-source directional-disagreement term before the hidden-state update. After elapsed `warmupBuckets × bucketSeconds` warmup, the detector fires on positive standardized innovation crossing the configured trigger, with `freshCapSeconds` per-market delta cap and the `is_heartbeat`/`0.500` opening-anchor sanitations preserved in the upstream bucket build. The default live anchor profile is `baselineMode="opening-ramp"`: it starts from `openingBaselineBuckets × bucketSeconds` elapsed game context and reaches rolling memory at `openingRampCompleteBuckets × bucketSeconds`. `baselineMode="historical-blend"` starts from last-five same-side historical priors, fades by NBA game-clock elapsed time, uses `trailingGameMinutes` for current-game memory, and blends a short `recentWallMinutes` wall-clock tack-on with `recentWallWeight`. Advanced hidden-state, breadth, observation-embedding, observation-noise, and variance-adaptation coefficients live in the structured `stateSpace` object.
