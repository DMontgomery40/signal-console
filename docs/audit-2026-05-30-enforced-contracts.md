...
- The board state-space signal pools implied-probability deltas from **every**
  ...
  support: `docs/design-language.md:223` ("how much every market on a game
  wiggles") plus owner confirmation; the root `PRD.md` is historical context,
  not the controlling spec for this contract.
...
Python-fail-not-skip tests are written and passing; F-006's guard is covered by a
behavioral test on the market-anomaly path
(`live-repository.test.ts` "guards out-of-range implied_probability").
