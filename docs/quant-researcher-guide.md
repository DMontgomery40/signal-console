...
`--limit N`, `--since ISO`, and `--until ISO` post-filter whatever was selected on any
path. `--seed N` and `--out <dir>` are also accepted (default seed 42, default out root
`outputs/nba-quant-lab/snapshots`). Sampling is **never** implicit — omitting `--sample`
gives you the full corpus.
