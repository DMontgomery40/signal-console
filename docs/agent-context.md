...
- Research lab integrated (PR #9, merged 2026-06-08): the NBA miscredit-attribution research lab (sidecar `research/` re-ranker + FAR + separation + confluence, harvester scripts, `/v1/research/attribution` API + ResearchPage portal, `harvestMiscreditLabels` in `live-repository`) lives on `main`, with audit fixes F-007, F-008 (`sourceMarkets` -> `contributingSourceKeys`), F-009 (aliases collapsed — `band`/`score` canonical), F-011 (verify-citations guard), and F-012 (`resolveOddsApiKey` honors the preferred `ODDSAPI_API_KEY`). F-010 (board-anomaly cluster only half-wired) is the one documented open finding: a root-cause+debugger pass found the cluster is partly live via `normalizeBoardText`, so removal is unsafe, and with no live games (offseason) the quiet unwired path is harmless; revisit wire-vs-rewrite later.
- Branch-parity consolidation (2026-06-09): the remaining real fixes on `preserve/main-statespace-20260529` were ported onto `main` — F-006 `implied_probability BETWEEN 0 AND 1` guard, F-004 contract-derived Sensitivity dial range + `RotaryDial` out-of-range reconcile, the `player_prop_ticks.parquet` + `--control-ticks` exporter surface (closing the doc/Python-loader disconnect), explicit `missing` source-coverage rows, `RESEARCH_OUTPUT_ROOT` pass-through in `pnpm quant`, the F-007 detector-source contract test, and the full 2026-05-30 audit ledger/finding docs. The `preserve/*` and `ralph/*` branches were archived as tags (`archive/preserve-main-statespace-20260529`, `archive/ralph-signal-console-v2`) and deleted; `main` is the only living branch. The gold-DB sidecar LFS archive remains only on the preserve archive tag.
- Repo identity: `signal-console-mlb` and `signal-console-nfl` are SEPARATE projects, never authority here. Any other local `signal-console*` directory/worktree is a checkout of THIS repo; land its work on `main` via PR, then remove it.
