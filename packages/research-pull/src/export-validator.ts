 ...
 *  - scope "sample" rejects `gameIds`; explicit-game exports use scope "full".
...
export function validateExportRequest(input: unknown): ExportValidationResult {
  ...
  if (request.scope === "sample" && request.gameIds !== undefined && request.gameIds.length > 0) {
    errors.push({
      code: "sample_rejects_game_ids",
      message: 'scope "sample" cannot include `gameIds`; use scope "full" for explicit games.',
      path: "gameIds",
    });
  }
