...
const researchDir = join(repoRoot, "outputs", "innovation-team-suspend-signal-report", "research");
const hasResearchArtifacts = [
  "incident-registry-expanded.json",
  "incident-board-sheet.json",
  "incident-backtest-fixture.json",
].every((relativePath) => existsSync(join(researchDir, relativePath)));
...
describe("incident sheet contract", () => {
  it.skipIf(!hasResearchArtifacts)(
    "keeps the permanent registry, sheet, and fixture in sync",
    () => {
  ...
  it.skipIf(!hasResearchArtifacts)(
    "preserves the board-scoreability boundary for anchored incidents",
    () => {
      const sheet = loadJson("incident-board-sheet.json");
      const fixture = loadJson("incident-backtest-fixture.json");
      const rows = new Map((sheet.rows ?? []).map((row) => [String(row.id), row] as const));
