import type { BoardAnomalyAlert, MarketAnomalyAlert, MarketFamily } from "@signal-console/domain";
...
function buildMarketStructureReason(alert: MarketAnomalyAlert): string {
  ...
  const isPredictionMarket = sourceKindFor(alert.source) === "prediction-market";
  ...
  if (m.volumeShare != null) {
    const shareSource = m.finalMarketVolume != null ? "FINAL-volume (forensic)" : "live-to-date";
    parts.push(`${(m.volumeShare * 100).toFixed(1)}% volume share [${shareSource}]`);
  }
...
function fanoutShockKind(anchorAt: string, pbp: PlayByPlayContext): BoardAnomalyAlert["shockKind"] {
    ...
    for (let i = 0; i < list.length; i += 1) {
      ...
      fanouts.push({
        ...
        primaryAlert: cluster.reduce((best, member) =>
          (member.alert.metrics.volumeShare ?? 0) > (best.alert.metrics.volumeShare ?? 0)
            ? member
            : best,
        ).alert,
        members: cluster,
...
function buildFanoutReason(fanout: Fanout): string {
  ...
  const pairedSummary =
    fanout.pairedParticipants.size > 0
      ? ` Other player markets moved alongside: ${Array.from(fanout.pairedParticipants.keys())
          .slice(0, 3)
          .map(titleCase)
          .join(", ")}.`
      : "";
...
export function fanoutToBoardCard(
  ...
  const peakShare = Math.max(...fanout.members.map((m) => m.alert.metrics.volumeShare ?? 0));
  const totalNotional = fanout.members.reduce((sum, m) => sum + (m.alert.metrics.notional ?? 0), 0);
  ...
  const confidence = Math.min(0.97, 0.6 + peakShare * 0.3 + fanout.members.length * 0.03);
  ...
  const evidence: BoardAnomalyAlert["evidence"] = fanout.members
    .slice()
    .sort((a, b) => (b.alert.metrics.volumeShare ?? 0) - (a.alert.metrics.volumeShare ?? 0))
    .slice(0, 8)
    .map((member) => ({
      ...
      contribution: Number(Math.min(1, member.alert.metrics.volumeShare ?? 0).toFixed(3)),
  ...
  if (fanout.pairedParticipants.size > 0) {
    drivers.push(`${fanout.pairedParticipants.size} paired-player marker(s) in surrounding window`);
  }
  ...
  return {
    ...
    inspect: {
      ...
      relationFamilies: Array.from(new Set(fanout.members.map((m) => m.family ?? "other"))),
    },
    playByPlay: pbp,
    vigAdjusted: null,
  };
...
export function marketAnomalyToBoardCard(
  ...
  if (alert.metrics.finalMarketVolume != null) {
    drivers.push("volume-share computed against FINAL market volume (forensic, not live)");
  }
  if (!pbp.available) {
  ...
  return {
    ...
    components: {
      residual: Number(((alert.components.offPrice + alert.components.volatility) / 2).toFixed(3)),
